{{ config(
    ...
    unique_key='bike_station_id || last_rate_date || file_name'
) }}

WITH source AS (
    SELECT *
    FROM {{ source('bronze', 'brz_station_details') }}
),
...
{% if is_incremental() %}
-- Ne charger que les nouvelles lignes
WHERE bike_station_id || last_rate_date || file_name NOT IN (
    SELECT bike_station_id || last_rate_date || file_name FROM {{ this }}
)
{% endif %}
